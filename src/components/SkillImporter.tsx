...
import { Upload, Folder, FileJson, Check, AlertCircle, X, Terminal, Loader2, FileCode, FileText } from 'lucide-react';
...
export function SkillImporter({ 
  ...
  const categorizeFile = async (file: File): Promise<'Invoices' | 'Development' | 'Skills' | 'EdgeGallery' | 'Unknown'> => {
    const name = file.name.toLowerCase();
    
    // Check for Edge Gallery SKILL.md
    if (name === 'skill.md' || name.endsWith('/skill.md')) {
      return 'EdgeGallery';
    }
    
    if (name.endsWith('.json') || name.endsWith('.json.txt')) {
      try {
        const content = await file.text();
        const json = JSON.parse(content);
        if (json.name && (json.instructions || json.systemPrompt)) return 'Skills';
        if (json.skill && json.skillMd) return 'EdgeGallery';
      } catch (e) {}
    }
    ...
    for (let i = 0; i < files.length; i++) {
      ...
      if (category === 'Skills' && file.name.endsWith('.json')) {
        try {
          ...
          // Check for Edge Gallery export format
          if (json.skill && json.skillMd) {
            const partial = parseSkillMd(json.skillMd);
            const skill: Skill = {
              id: json.skill.id || generateId(),
              name: json.skill.name || partial.name || 'Imported Skill',
              description: json.skill.description || partial.description || 'Imported from Edge Gallery.',
              instructions: partial.instructions || json.skill.instructions || '',
              examples: Array.isArray(json.skill.examples) ? json.skill.examples.map((ex: any) => ({
                id: ex.id || generateId(),
                input: ex.input || '',
                output: ex.output || ''
              })) : [],
              author: json.skill.author || 'Imported',
              authorId: json.skill.authorId || 'imported',
              createdAt: json.skill.createdAt || Date.now(),
              updatedAt: Date.now(),
              difficulty: json.skill.difficulty || 50,
              tags: Array.isArray(json.skill.tags) ? json.skill.tags : [],
              version: json.skill.version || '1.0.0',
              isPublic: false,
              schedule: json.skill.schedule || { enabled: false, frequency: 'manual' },
              edgeGallery: json.skill.edgeGallery
            };
            newCandidates.push({ file, skill, category: 'EdgeGallery' });
          } else {
            const skill: Skill = {
      ...
      } else if (category === 'EdgeGallery') {
        try {
          const content = await file.text();
          const parsed = parseSkillMd(content);
          const skill: Skill = {
            id: generateId(),
            name: parsed.name || `Imported ${new Date().toLocaleDateString()}`,
            ...
            edgeGallery: parsed.edgeGallery
          };
          newCandidates.push({ file, skill, category: 'EdgeGallery' });
        } catch (err) {
          newCandidates.push({ file, skill: null, category: 'Unknown', error: 'Failed to parse SKILL.md' });
        }
      } else {
...
<div className="grid grid-cols-1 md:grid-cols-3 gap-4 h-[400px]">
            ...
            <div className="flex flex-col border border-border bg-primary/5 overflow-hidden">
               <div className="p-2 border-b border-border bg-primary/10 flex items-center justify-between">
                  <span className="text-[8px] font-mono uppercase tracking-widest font-bold text-primary flex items-center gap-1">
                    <FileCode className="w-3 h-3" />
                    Edge Gallery
                  </span>
                  <span className="text-[8px] font-mono opacity-50">{candidates.filter(c => c.category === 'EdgeGallery').length} Files</span>
               </div>
               <ScrollArea className="flex-1">
                  <div className="p-2 space-y-1">
                    {candidates.filter(c => c.category === 'EdgeGallery').map((c, i) => (
                      <div key={i} className="flex items-center justify-between p-1.5 bg-primary/10 border border-primary/20">
                        <div className="flex items-center gap-1.5 truncate">
                          <FileText className="w-3 h-3 text-primary flex-shrink-0" />
                          <span className="text-[9px] font-mono truncate">{c.file.name}</span>
                        </div>
                        <span className="text-[7px] font-mono bg-primary/20 text-primary px-1 flex-shrink-0">SKILL</span>
                      </div>
                    ))}
                    {candidates.filter(c => c.category === 'EdgeGallery').length === 0 && (
                      <p className="text-[8px] font-mono text-muted-foreground italic p-2">No SKILL.md found</p>
                    )}
                  </div>
               </ScrollArea>
            </div>
             ...
             <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
                {candidates.filter(c => c.category === 'Skills').map((c, i) => (
                  <div key={i} className="flex items-center justify-between p-2 bg-emerald-500/5 border border-emerald-500/10">
                    <span className="text-[9px] font-mono truncate">{c.file.name}</span>
                    <span className="text-[7px] font-mono bg-emerald-500/10 text-emerald-500 px-1">JSON</span>
                  </div>
                ))}
                {candidates.filter(c => c.category === 'EdgeGallery').map((c, i) => (
                  <div key={i} className="flex items-center justify-between p-2 bg-primary/5 border border-primary/20">
                    <span className="text-[9px] font-mono truncate">{c.file.name}</span>
                    <span className="text-[7px] font-mono bg-primary/10 text-primary px-1">EDGE</span>
                  </div>
                ))}
             </div>
          </div>
