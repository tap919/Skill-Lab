...
import { Edit2, Trash2, Code2, Download, Share2, Tag, Sparkles, Clock, Workflow, Package, FileCode } from 'lucide-react';
import { Skill } from '../types';
import { Card, CardContent, CardDescription, CardFooter, CardHeader, CardTitle } from '@/components/ui/card';
import { Button } from '@/components/ui/button';
import { Badge } from '@/components/ui/badge';
import { downloadSkillForEdgeGallery, exportSkillForEdgeGallery } from '../services/edgeGalleryExporter';
    ...
    <Card className="rounded-none border-border bg-card/40 hover:bg-card transition-all group overflow-hidden relative">
      ...
      <CardFooter className="pt-0 flex flex-col gap-2">
        <div className="flex justify-between items-center gap-2">
          ...
          <div className="text-[9px] font-mono text-muted-foreground flex items-center gap-1">
            ...
            <span className="flex items-center gap-0.5">
              ...
              {skill.edgeGallery?.isJsSkill ? 'EG' : 'TEXT'}
            </span>
          </div>
        </div>
        <div className="flex flex-wrap gap-1.5">
          ...
          <Button variant="outline" size="sm" className="h-8 font-mono text-[10px] uppercase" onClick={exportSkillMd}>
          ...
          {onRemix && (
            <Button 
              variant="outline" 
              size="sm" 
              className="h-8 font-mono text-[10px] uppercase ml-auto"
              onClick={onRemix}
            >
